...
<div class="row g-3">
    ...
    <div class="col-md-6 col-xl-3">
        <div class="card p-4 h-100">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h3 class="h6 mb-0">Ozalit Modülü</h3>
                <span class="badge {{ $vendor->hasActiveOzalitModule() ? 'bg-success-subtle text-success' : 'bg-warning text-dark' }}">
                    {{ $vendor->hasActiveOzalitModule() ? 'Aktif' : 'Pasif' }}
                </span>
            </div>
            <p class="small mb-2">Aylık ücret: <strong>₺{{ number_format($ozalitMonthlyFee ?? 199, 2, ',', '.') }}</strong></p>
            <p class="small text-muted">Bitiş: {{ optional($vendor->ozalit_expires_at)->format('d.m.Y H:i') ?? '-' }}</p>
