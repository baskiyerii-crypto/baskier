...
<a href="{{ route('vendor.outdoor.requests.index') }}" class="small">← Liste</a>
...
<ul>
    @foreach($vendorRequest->plan->items->where('owner_vendor_id', $vendorRequest->vendor_id) as $item)
...
<form method="POST" action="{{ route('vendor.outdoor.requests.quote', $vendorRequest) }}" class="card p-3 mb-3">
    @csrf
...
<form method="POST" action="{{ route('vendor.outdoor.requests.decline', $vendorRequest) }}" onsubmit="return confirm('Reddedilsin mi?');">@csrf<button class="btn btn-outline-danger btn-sm">Reddet</button></form>
@endif
