...
@section('content')
                        ...
                        <a href="{{ route('products.index', ['category' => $category->slug]) }}" class="group by-card by-card-hover overflow-hidden">
                            <div class="aspect-4/3 bg-slate-100 flex items-center justify-center">
                                @if(!empty($category->image))
                                    <img src="{{ \App\Support\MediaUrl::public($category->image) }}" alt="{{ $category->localizedName() }}" class="h-full w-full object-cover transition group-hover:scale-[1.02]">
                                @else
                                    <span class="text-3xl font-extrabold text-slate-300">{{ mb_strtoupper(mb_substr($category->localizedName(), 0, 1)) }}</span>
                                @endif
                            </div>
                            <div class="p-4">
    ...
    <section class="by-container py-12">
        <div class="flex flex-wrap items-end justify-between gap-4">
            ...
            <a href="{{ route('quote-requests.create', ['type' => 'freelancer']) }}" class="by-btn-secondary">{{ __('home.all_jobs') }}</a>
        </div>
            ...
            <div class="mt-6 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-5">
                @foreach($freelancerCategories ?? [] as $cat)
                    <div class="by-card by-card-hover overflow-hidden">
                        <a href="{{ route('quote-requests.create', ['type' => 'freelancer']) }}" class="block">
                            <div class="aspect-4/3 bg-slate-100 flex items-center justify-center">
                                <span class="text-2xl font-extrabold text-slate-300">{{ mb_strtoupper(mb_substr($cat['label'], 0, 1)) }}</span>
                            </div>
                            <div class="p-4">
                        ...
                        <div class="p-4 pt-0">
                            <a href="{{ route('quote-requests.create', ['type' => 'freelancer']) }}" class="w-full by-btn-primary">{{ __('home.path_freelancer_cta') }}</a>
                        </div>
                    </div>
                @endforeach
            </div>
