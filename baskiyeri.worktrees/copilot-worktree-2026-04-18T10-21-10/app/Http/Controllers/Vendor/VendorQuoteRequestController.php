...
class VendorQuoteRequestController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = QuoteRequest::with(['category', 'user'])
            ->where('status', 'open')
    ...
    public function show(Request $request, QuoteRequest $quoteRequest)
    {
        ...
        if (! $categoryIds->contains($quoteRequest->category_id)) {
            abort(403, 'Bu kategoriye teklif veremezsiniz.');
        }
        $quoteRequest->load(['category', 'user', 'quotes' => fn ($q) => $q->where('vendor_id', $vendor->id)]);
